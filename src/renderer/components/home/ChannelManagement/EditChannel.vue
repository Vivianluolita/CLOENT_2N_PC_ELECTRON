...
export default {
  ...
  destroyed () {
    ipcRenderer.removeListener(
      'update-channel-info-res',
      this.updateChannelInfoRes
    )
    ipcRenderer.removeListener(
      'get-edit-channel-info-res',
      this.getEditChannelInfoRes
    )
  },
  created () {
    // 编辑上传
    ipcRenderer.on('update-channel-info-res', this.updateChannelInfoRes)
    // 房间数据获取
    ipcRenderer.on('get-edit-channel-info-res', this.getEditChannelInfoRes)
  },
  mounted () {},
  methods: {
    updateChannelInfoRes (event, params) {
      ...
      if (params.data && params.data.code === 0) {
        ...
        if (params.data.data.tags) {
          ...
          params.data.data.tags.map((item, index) => {
            ...
            // this.form.type_channel = this.channeltpye.toString()
          })
        }
      }
    },
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (tags.length === 0 && this.form.type_channel.toString() !== '0') {
            ...
            let channelData = {
              channel_no: this.$store.state.home.selectedChannelNo,
            ...
            ipcRenderer.send('update-channel-info', channelData)
          }
        } else {
